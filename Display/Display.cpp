...
static DISPLAYPARAMS   TempParams;
...
extern "C" long WINAPI   USERUI( void *ref, char *parameters, HWND parent )
{
    ...
    memcpy(&TempParams,parameters,sizeof(DspPar));
    res = DialogBox(hInst,MAKEINTRESOURCE(IDD_PARAMETERS),
                    parent,(DLGPROC)DlgProc);

    if( res==1 )
        memcpy(parameters,&TempParams,sizeof(DspPar));
...
BOOL CALLBACK    DlgProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    {
        case WM_INITDIALOG:
            SetDlgItemInt(hDlg,IDC_FFTSIZE,TempParams.fftsize,FALSE);
            SetDlgItemInt(hDlg,IDC_FFTASCALE,TempParams.ascale,FALSE);
            SetDlgItemInt(hDlg,IDC_FFTFSCALE,TempParams.fscale,FALSE);
            SetDlgItemInt(hDlg,IDC_FFTWINDOW,TempParams.wtype,FALSE);
            bReturnValue = TRUE;
            break;
        ...
        case WM_COMMAND:
        {
            ...
            {
                case IDOK:
                    value = GetDlgItemInt(hDlg,IDC_FFTSIZE,&valid,FALSE);
                    if( valid &&
                        !(value==64 || value==128 || value==256 || value==512 ||
                          value==1024 || value==2048 || value==4096 || value==8192 ||
                           value==16384) )
                        valid = 0;
                    if( valid )
                        TempParams.fftsize = value;
                    else
                    {
                        SetFocus(GetDlgItem(hDlg,IDC_FFTSIZE));
                        goto fail;
                    }

                    value = GetDlgItemInt(hDlg,IDC_FFTASCALE,&valid,FALSE);
                    if( valid &&
                        !(value==0 || value==1 || value==2) )
                        valid = 0;
                    if( valid )
                        TempParams.ascale = value;
                    else
                    {
                        SetFocus(GetDlgItem(hDlg,IDC_FFTASCALE));
                        goto fail;
                    }

                    value = GetDlgItemInt(hDlg,IDC_FFTFSCALE,&valid,FALSE);
                    if( valid &&
                        !(value==0 || value==1) )
                        valid = 0;
                    if( valid )
                        TempParams.fscale = value;
                    else
                    {
                        SetFocus(GetDlgItem(hDlg,IDC_FFTFSCALE));
                        goto fail;
                    }

                    value = GetDlgItemInt(hDlg,IDC_FFTWINDOW,&valid,FALSE);
                    if( valid &&
                        !(value==0 || value==1 || value==2) )
                        valid = 0;
                    if( valid )
                        TempParams.wtype = value;
                    else
                    {
                        SetFocus(GetDlgItem(hDlg,IDC_FFTWINDOW));
                        goto fail;
                    }
fail:
                    if( !valid )
